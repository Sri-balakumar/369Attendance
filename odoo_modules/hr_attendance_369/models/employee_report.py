...
class EmployeeReport(models.Model):
    ...
    @api.depends('summary_line_ids.leave_deduction',
    ...
    def _generate_data(self):
        ...
        for emp in employees:
            ...
            def calc_leave_deduction_live(leave_record, _emp=emp):
                ...
                Daily rate = monthly wage ÷ working days in the month (the same
                `total_working_days` figure this report computes, which excludes
                Sundays and public holidays). This makes a full month of unpaid
                leave wipe the wage exactly, and keeps the report consistent
                with the absence rate.
                """
                ...
                if not leave_cfg or not leave_cfg.unpaid_leave_deduction_enabled:
                    return 0.0
                _wage = _emp.monthly_wage or 0.0
            ...
            # The configured weekdays, minus public holidays. Sundays are out
            # because Sunday is unchecked in the working-days boxes, and
            # holidays are excluded here — which is exactly what makes both of
            ...
            # day / absent) against the configured office-hour thresholds. Read
            # it here so the day-wise lines can show the graded label - which is
            # what carries the bracketed start time for somebody who began late
            # but owes nothing - and so half days reduce what the month earned.
            #
            # Absent days were never in present_dates, so they were never
            # earned in the first place — the report needs no absence deduction
            # of its own.
            day_status_info = {}
            for _ds in self.env['hr.attendance.day.status'].sudo().search([
                    ('employee_id', '=', emp.id),
                    ('date', '>=', d_from),
                    ('date', '<=', d_to),
            ]):
                day_status_info[_ds.date] = _ds

            # ── LATE DATA ───────────────────────────────────
            # Two filters replace what `late_sequence > 0` used to encode:
            #   * _past_late_window  — an arrival after the late window is
            #     graded Present and costs nothing, so it is not a "late day";
            #   * _is_day_first_checkin — the arrival is counted once.
            # Applied in Python because both are office-timezone decisions the
            # ORM cannot express in a domain.
            late_candidates = self.env['hr.attendance'].search([
                ('employee_id', '=', emp.id),
                ('is_late', '=', True),
                ('late_minutes', '>', 0),
                ('date', '>=', d_from),
            ...
            late_records = late_candidates.filtered(
                lambda a: not a._past_late_window(config_data)
                and a._is_day_first_checkin(config_data)
            )

            all_late_times = 0
            late_times_charged = 0
            ...
            #
            # Derived from the day-status rows, NOT collected while the day-wise
            # lines are built. It used to be gathered inside the `is_present`
            # branch, which meant a late arrival that ALSO worked under the
            # half-day ratio took the `has_late` branch and was never added --
            # so it was docked half a day on its day-status row while still
            # earning a full day here. Which detail line a day happens to render
            # as must not decide what it is worth.
            #
            # Leave dates are excluded so a leave day is never netted twice; the
            # leave accounting below already prices those.
            ladder_half_dates = {
                d for d, ds in day_status_info.items()
                if ds.status == 'half_day' and d not in leave_date_info
            }
            ...
            while current_date <= d_to:
                ...
                if is_holiday:
                    ...
                    for att in late_atts:
                        check_in_str = _office_time(att.check_in)
                        check_out_str = _office_time(att.check_out)
                        ds_late = day_status_info.get(current_date)
                        desc_parts = [ds_late.status_display if ds_late
                                      and ds_late.status_display else 'Late']
                    ...
                    # Normal present day. all_attendance is ordered by
                    # check_in asc, so the first row for this date IS the
                    # earliest arrival — which is the one that grades the day.
                    first_att = None
                    for att in all_attendance:
                    ...
                    # What the ladder made of this day. `status_display` is
                    # the label plus the real start time in brackets whenever
                    # the person started late - including the case that costs
                    # nothing, which is exactly the one that would otherwise be
                    # invisible on this report.
                    ds = day_status_info.get(current_date)
                    is_ladder_half = bool(ds) and ds.status == 'half_day'

                    if is_ladder_half:
                        base_label = ds.status_display or 'Half Day'
                    else:
                        base_label = (ds.status_display if ds else '') or 'Present'
                    ...
                    detail_vals.append({
                        ...
                        'department_name': dept_name,
                        'entry_date': current_date,
                        'day_type': 'half_day' if is_ladder_half else 'present',
            ...
            # ── WAGE ─────────────────────────────────────────
            emp_wage = emp.monthly_wage or 0.0
            ...
            # days plus any PAID leave. Absent days and
            ...
            if earned > emp_wage:
                earned = emp_wage
            # Lateness costs nothing now, so earned IS the final amount before
            # the leave split below. Only half days (already netted out of
            # earned_days) and unpaid leave reduce pay.
            final_amt = round(earned, 2)
            # Show the unpaid-LEAVE deduction on its own column. Total Deduction is
            # then everything else not received (absent days, half days). So the
            # three reconcile to the wage:
            #   Total Deduction + Leave Deduction + Final = Wage.
    ...
    def action_print_pdf(self):
        return self.env.ref(
            'hr_attendance_369.action_report_employee_pdf'
        ).report_action(self)
    ...
    def action_export_excel(self):
        ...
        detail_widths = [12, 22, 18, 10, 25, 12, 12, 10, 20, 12, 14, 20, 12]
...
class EmployeeReportSummaryLine(models.Model):
    ...
    def action_view_day_details(self):
        ...
        view_id = self.env.ref('hr_attendance_369.view_detail_line_list').id
        search_view_id = self.env.ref('hr_attendance_369.view_detail_line_search').id
        return {
