...
class AttendanceLateSummaryWizard(models.TransientModel):
    ...
    def action_generate_summary(self):
        ...
        Counts every chargeable late day in the month.
        """
        self.ensure_one()
        ...
        for att in attendances:
            # Only the day's first arrival is graded and charged, and lateness
            # past the late window costs nothing -- so those are not late days.
            # Both are office-timezone decisions, hence the Python filter.
            config_data = Config.get_config_for_employee(att.employee_id.id)
            if att._past_late_window(config_data):
                continue
            if not att._is_day_first_checkin(config_data):
                continue
            ...
            # No monthly grace allowance: every chargeable late day counts.
